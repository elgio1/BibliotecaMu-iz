<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/admin.css">
    <title>BiblioTECa</title>
</head>
<body>

    <h2>Bienvenido Administrador</h2>
    <p>Selecciona la opción que deseas realizar</p>
    <br><br>
    <div class="container">

        <div class="container-image">
            <img src = "../imagenes/user_actualizar.jpg" alt="">
            <div class="content">
                <p>
                    <span>Actualizar Usuarios</span>
                    El panel Actualizar Usuarios le permite editar los usuarios de la base de datos
                </p>
            </div>
        </div>

        
        <div class="container-image">
            <img src = "../imagenes/user_delate.jpg" alt="">
            <div class="content">
                <p>
                    <span>Eliminar Usuarios</span>
                    El panel Eliminar Usuarios le permite borrar los usuarios disponibles en la base de datos
                </p>
            </div>
        </div>

        <br><br>
        <div class="container-image">
            <img src = "../imagenes/libro_actualizar.jpg" alt="">
            <div class="content">
                <p>
                    <span>Actualizar Libros</span>
                    El panel Actualizar Libros le permite editar los libros disponibles en la base de datos
                </p>
            </div>
        </div>

        
        <div class="container-image">
            <img src = "../imagenes/libro_agregar.jpg" alt="">
            <div class="content">
                <p>
                    <span>Agregar Libros</span>
                    El panel Agregar Libros le permite añadir libros a la base de datos
                </p>
            </div>
        </div>

        
        <div class="container-image">
            <img src = "../imagenes/libro_eliminar.jpg" alt="">
            <div class="content">
                <p>
                    <span>Eliminar Libros</span>
                    El panel Eliminar Libros le permite borrar los libros disponibles en la base de datos
                </p>
            </div>
        </div>

    </div>

<?php
    require "footer.php";
?>
</body>
</html>